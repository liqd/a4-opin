...
EMAIL_PORT = mail_url.port or 143
EMAIL_HOST_USER = unqote(mail_url.username)
EMAIL_HOST_PASSWORD = mail_url.password
DEFAULT_FROM_EMAIL = unqote(mail_url.username)
...
if scheme == 'postgres':
    DATABASES = {
        'default': {
            ...
            'NAME': unqote(db_url.path.strip('/')),
            'USER': unqote(db_url.username),
            'PASSWORD': unqote(db_url.password),
