...
@pytest.mark.django_db
def test_authenticated_user_can_not_delete_rating(rating, user2, apiclient):
    url = reverse('ratings-detail', kwargs={'pk': rating.pk})
...
@pytest.mark.django_db
def test_creater_of_rating_can_set_zero(rating_factory, comment,  apiclient):
    ct = ContentType.objects.get_for_model(comment)
    rating = rating_factory(object_pk=comment.id, content_type=ct)
    url = reverse('ratings-detail', kwargs={'pk': rating.pk})
    apiclient.force_authenticate(user=rating.user)
...
@pytest.mark.django_db
def test_meta_info_of_rating(rating_factory, comment,  apiclient, user, user2):
    ct = ContentType.objects.get_for_model(comment)
    pk = comment.pk
    url = reverse('ratings-list')
