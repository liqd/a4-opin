...
*/
  function createLocation () {
    var wktFormat = transToWkt(selectedShape.getPath().b)
    selectedShape.setMap(null)
...
*/
  ...
  function drawPolygon (wktFormat) {
    var polygon = transToArray(wktFormat)
  ...
  function transToArray (wkt) {
    // initialize wkt-textarea-input
    var res = []
    var coors

    var inputCache = wkt.replace('POLYGON((', '')
    inputCache = inputCache.replace('))', '')

    var inputArray = inputCache.split(',')

    for (var j = 0; j < inputArray.length - 1; j++) {
      coors = inputArray[j].trim().split(' ')
  ...
  window.initialize = function () {
    ...
    google.maps.event.addListener(drawingManager, 'overlaycomplete', function (event) {
      /* 'radius' is assigned a value but never used
...
*/
  ...
  // function removeChoice(choices, qcorderIdIn, qorderId, key) {
    if (choices.length > 1) {
      ...
      elem = document.getElementById(remvkey)
      elem.parentNode.removeChild(elem)
